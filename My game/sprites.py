...
class Cooldown():
    ...
    def timer(self):
        self.current_time = floor((pg.time.get_ticks())/1000)
# Create a class defining the attributes of the player
class Player(Sprite):
    ...
    def update(self):
        ...
        #checks if player hits the trophy
        '''
        thits = pg.sprite.spritecollide(self, Trophy, False)
        if thits:
            thits[0].tagged = True
            thits[0].ced.event_reset()
            pg.quit()
         ''' 
...
#Instantiate the trophy as a prize for the player
class Trophy(Sprite):
    def __init__(self, game, x, y, img_file):
        Sprite.__init__(self)
        self.game = game
        self.img_file = img_file
